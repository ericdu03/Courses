...
def choose(paragraphs, select, k):
    ...
    for i in range(0, len(paragraphs)):
        if select(paragraphs[i]):
           paragraphs2 += [paragraphs[i]]
    if k <len(paragraphs2):
        return paragraphs2[k]
    else:
        return ''
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def helper(input):
        sentence = remove_punctuation(input)
        lowercased = lower(sentence)
        words = split(lowercased)
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    def helper(typed_updated, reference_updated, count=0):
        if len(typed_updated) == 0:
            return count/len(typed_words)*100
        elif len(reference_updated) == 0:
            return count/len(typed_words)*100   
        if typed_updated[0] == reference_updated[0]:
            return helper(typed_updated[1:], reference_updated[1:], count+1)   
        else:
            return helper(typed_updated[1:], reference_updated[1:], count)       
    ...
    if len(typed_words) == 0:
        return 0.0
    return helper(typed_words, reference_words)
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    compare = limit+1
    memory = valid_words[0]
    for i in range(0, len(valid_words)):
        if diff_function(typed_word, valid_words[i], limit) < compare:
            compare = diff_function(typed_word, valid_words[i], limit) #gives smallest number that diff_function spits out
            memory = valid_words[i]
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def helper(start, goal, counter=0):
        if counter > limit:
            return limit+1
        if len(start) == 0 or len(goal) == 0:
            return abs(len(start) - len(goal)) + counter
        if start[0] != goal[0]:
            return helper(start[1:], goal[1:], counter+1)
        else:
            return helper(start[1:], goal[1:], counter)
...
def time_per_word(words, times_per_player):
    ...
    for i in range(0, len(times_per_player)):
        ...
        for j in range(0, len(times_per_player[i])-1):
            counter+=[times_per_player[i][j+1] - times_per_player[i][j]]
        counter = []
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            ...
            if low > curr:
                low = curr
                player = j
        result[player].append(get_words(match)[i])
    return result
    # END PROBLEM 10
