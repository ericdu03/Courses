...
def pingpong(n):
    ...
    def helper(k, a=1):
        ...
        if k%8 == 0 or num_eights(k) > 0:
            return helper(k-1, a+1) + (-1)**a
        else:
            return helper(k-1, a) + (-1)**a
    return helper(n)
...
def count_coins(change):
    ...
    #         return 0
    #     else:
    #         return helper(k, ascending_coin(coin)) + helper(k -coin, ascending_coin(coin)) + helper(k-coin, coin)
    # return helper(change)

    def helper(k, coin=25):
        if coin == None:
            return 0
        if k == 0:
            return 1
            ...
            return helper(k, descending_coin(coin)) + helper(k-coin, coin)
    return helper(change)
