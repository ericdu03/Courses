...
\begin{document}
	...
	\begin{solution}
		Let \( A_1 = \{ v + u_1 : u_1 \in U_1 \} \), and \( A_2 = \{ v + u_2 : u_2 \in U_2 \} \).
		Then, because \( v \) is common between both sets, then elements in the intersection \( A_1 \cap A_2
		\) satisfy \( v + u_1 = v + u_2 \), or in other words \( u_1 = u_2 \). Let the set of \( u_i \) that
		satisfy this be grouped into a new set, \( U_3 \). The zero element is \( U_3 \), since \( v = v \).
		\( U_3 \) is closed under addition, since the equality \( u_1 = u_2 \) implies that the addition of
		any two elements would also satisfy \( u_1 = u_2 \), and same goes with scalar multiplication. Thus,
		\( U_3 \) is a subspace. The set \( A_1 \cap A_2 \) can then be written as \( v + U_3 \), hence it is
		a translate of a subspace. If the intersection is empty, then \( A_1 \cap A_2 = \varnothing \).   
	\end{solution}

	\begin{problem}
		Suppose \( U = \{(x_1, x_2, \dots, ) \in \F^{\infty} \mid x_k \neq 0 
		\text{ for finitely many \( k \)}\}  \). 
		\begin{enumerate}[label=\alph*)]
			\item Show that \( U \) is a subspace of \( \F^{\infty} \).

				\begin{solution}
					\( U \) is a subspace of \( \mathbf{F}^{\infty} \), since addition and scalar
					multiplication would only affect the nonzero entries, hence the resulting element would
					still be in \( U \).    
				\end{solution}
			\item Prove that \( \F^{\infty} / U \) is infinite-dimensional

				\begin{solution}
					Well, \( v \in \mathbf{F}^{\infty} \), so the only possible way to write down all the
					available translates is to use an infinite number of "basis vectors" (essentially
					infinite \( v \)'s), so \( \mathbf{F}^{\infty} / U \) is indeed infinite-dimensional.    
				\end{solution}
		\end{enumerate}
	\end{problem}
